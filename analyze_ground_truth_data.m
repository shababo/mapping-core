...
hold on
scatter3(result_ground_truth_set_fix(2).targ_pos(:,1), result_ground_truth_set_fix(2).targ_pos(:,2), -result_ground_truth_set_fix(2).targ_pos(:,3),100,'.k'); axis image
% set(gca,'Color','k')
view(60,30)
xlim([-30 60])
ylim([-60 0])
...
%     hold on
    ...
    for jj = 1:size(unique_targs,1)
        loc_times = result_ground_truth_set_fix(j).event_times_c1(these_trials' & unique_targs_trial_idx == jj);
...
paired_trials = ~isnan(result_ground_truth_set_fix(2).spike_times_c2') & ~isnan(result_ground_truth_set_fix(2).event_times_c1) & result_ground_truth_set_fix(2).event_times_c1 > 50 & result_ground_truth_set_fix(2).event_times_c1 < 140 & result_ground_truth_set_fix(2).targ_power < 50;
...
histogram(result_ground_truth_set_fix(2).event_times_c1(paired_trials)'...
    - result_ground_truth_set_fix(2).spike_times_c2(paired_trials),30:5:80,'Normalization','pdf')
hold on
x_vec = 20:.1:80;
